 ...
 * @author SEABIRDS-PC
 */
@WebServlet(name = "SVMWithRF", urlPatterns = {"/SVMWithRF"})
public class SVMWithRF extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        double svmaccuracy=0,svmprecision=0,svmrecall=0,svmf1score=0;
        ...
        try
        {            
            String fname=getServletContext().getRealPath("/Positive.txt");
        ...
        try
        {            
            String fname=getServletContext().getRealPath("/Negative.txt");
            File fe2=new File(fname);
            FileInputStream fis2=new FileInputStream(fe2);
            byte data2[]=new byte[fis2.available()];
            fis2.read(data2);
        ...
        try
        {            
            String fname=getServletContext().getRealPath("/stopwords1.txt");
            File fe2=new File(fname);
            FileInputStream fis2=new FileInputStream(fe2);
            byte data2[]=new byte[fis2.available()];
            fis2.read(data2);
        ...
        session.setAttribute("svmaccuracy", svmaccuracy);
        session.setAttribute("svmprecision", svmprecision);
        session.setAttribute("svmrecall", svmrecall);
        session.setAttribute("svmf1score", svmf1score);
        
        String svmwithrfResults=sb.toString().trim();
        ...
        session.setAttribute("svmwithrfResults", svmwithrfResults);
